...
(async () => {
  ...
  const { enabled = true } = await browser.storage.local.get(DEFAULTS);
  if (enabled) inject();

  // React live to toolbar toggle
...
<div class="panel" id="a11y-panel" role="dialog" aria-modal="true" aria-labelledby="a11y-title">
  ...
  <style>
    #sw-powered .pill{pointer-events:auto;display:flex;align-items:center;gap:.55rem;padding:.55rem .9rem;border-radius:999px;background:rgba(15,18,23,.6);backdrop-filter:saturate(140%) blur(8px);border:1px solid rgba(255,255,255,.12); box-shadow:0 8px 28px rgba(0,0,0,.28), inset 0 0 0 1px rgba(255,255,255,.05);font:600 12.5px/1.2 ui-sans-serif,system-ui,-apple-system,Segoe UI,Roboto,Arial,"Noto Sans","Helvetica Neue",sans-serif;color:#eaf1ff;}
    ...
    #sw-powered .brand{font-weight:800;letter-spacing:.3px}
    #sw-powered .brand.ship{background:linear-gradient(90deg,#22d3ee,#38bdf8,#60a5fa);-webkit-background-clip:text;background-clip:text;color:transparent;text-shadow:0 0 12px rgba(56,189,248,.25)}
    #sw-powered .brand.code{background:linear-gradient(90deg,#c084fc,#a78bfa,#f472b6);-webkit-background-clip:text;background-clip:text;color:transparent;text-shadow:0 0 12px rgba(192,132,252,.25)}
    #sw-powered .spark{width:.6rem;height:.6rem;border-radius:999px;flex:0 0 auto;background:linear-gradient(135deg,#22d3ee,#8b5cf6,#ec4899);box-shadow:0 0 12px rgba(139,92,246,.6),0 0 24px rgba(236,72,153,.35);animation:sw-pulse 2.4s ease-in-out infinite}
    @keyframes sw-pulse{0%,100%{transform:scale(1);filter:saturate(1)}50%{transform:scale(1.2);filter:saturate(1.35)}}
    @media (prefers-color-scheme: light){#sw-powered .pill{background:rgba(255,255,255,.75);color:#0b1220;border-color:rgba(0,0,0,.08)}#sw-powered .txt{opacity:.8}}
  </style>
...
`;
    ...
    document.addEventListener('selectionchange', () => {
      const autoselect = root.shadowRoot.querySelector('#a11y-autoselect');
      if (!autoselect || !autoselect.checked) return;
    ...
    $('#a11y-reset').addEventListener('click', () => {
      ...
      const keys = Object.keys(prefs); keys.forEach(k => delete prefs[k]);
    ...
    function handleVideoMute(on) {
      ...
      vids.forEach(v => { v.muted = !!on; if (on) { try { v.pause(); } catch {} } });
    }
    ...
    document.addEventListener('mousedown', (e) => {
      ...
      if (!path.includes(panel) && !path.includes(btnOpen) && !path.includes(root)) {
        panel.removeAttribute('open');
        backdrop.removeAttribute('open');
      }
    });
  ...
  function removeWidget() {
    try { window.__A11Y_WIDGET__ = false; } catch {}
    const r = document.getElementById('a11y-widget-root'); if (r) r.remove();
    const s = document.getElementById('a11y-global-css'); if (s) s.remove();
    const mask = document.getElementById('a11y-reading-mask'); if (mask) mask.remove();
    const btn = document.getElementById('a11y-hover-read-btn'); if (btn) btn.remove();
  }
})();